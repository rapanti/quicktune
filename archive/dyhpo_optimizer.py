...
from hpo4dl.configuration_manager.abstract_configuration_manager import (
...
class DyHPOOptimizer(AbstractOptimizer):
    ...
    def __init__(
        ...
        file_handler = logging.FileHandler(
            os.path.join(output_path, f"dyhpo_surrogate_{dataset_name}_{seed}.log")
        )
    ...
    def _predict(self) -> Tuple[np.ndarray, np.ndarray, List, List]:
        ...
        mean_predictions, std_predictions = self.model.predict_pipeline(
    ...
    def observe(self, configuration_results: List[Dict]):
        """
        Args:
        ...
        """
        assert (
            len(configuration_results) == 1
        ), "Dyhpo only support observe calls with only one observation."
    ...
    def generate_candidate_configurations(
        ...
        for hp_index in range(0, self.hp_candidates.shape[0]):
            if hp_index in self.examples:
                ...
                # pad it with zeros
                difference_curve_length = self.surrogate_config[
                    "cnn_kernel_size"
                ] - len(curve)
    ...
    def preprocess_hp_candidates(self) -> np.ndarray:
        ...
        Returns:
            preprocessed_candidates: np.ndarray
                ...
                preprocessed.
        """
        categorical_indicator, indicator_categories = (
            self.configuration_manager.get_categorical_indicator()
        )
        column_transformers = []
        numerical_columns = [
            col_index
            for col_index, category_indicator in enumerate(categorical_indicator)
        ...
        if categories is not None and len(categorical_columns) != 0:
            categories = [
                indicator_categories[col_index]
                for col_index, category_indicator in enumerate(categorical_indicator)
                if category_indicator
            ]
        ...
        if len(numerical_columns) > 0:
            if self.log_indicator is not None and any(self.log_indicator):
                log_columns = [
                    col_index
                    for col_index, log_indicator in enumerate(self.log_indicator)
                    if log_indicator
                ]
        ...
        column_transformers.append(
            (
                "feature_types_pre",
                ColumnTransformer(general_transformers, remainder="passthrough"),
            )
        )

        preprocessor = Pipeline(column_transformers)
