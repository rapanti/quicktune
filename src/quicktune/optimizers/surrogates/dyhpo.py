...
from quicktune.optimizers.surrogates.models import FeatureExtractor, CostPredictor
...
class DyHPO(Surrogate):
    ...
    def from_pretrained(cls, name_or_path: str) -> "DyHPO":
        ...
        """
        root = name_or_path
        config = yaml.safe_load(open(os.path.join(root, "config.yaml"), "r"))
        model = cls(config["extractor"], config["predictor"])
    ...
    def predict_pipeline(
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            self.gp.set_train_data(_train, target, False)

            _test = self.feature_extractor(**test_data)
            pred = self.gll(self.gp(_test))
